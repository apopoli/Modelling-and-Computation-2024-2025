function [phi]=FEM2D_s2(msh,p,t,BC)

% import mesh info
T = msh.TRIANGLES(:,1:3); % connectivity
x = msh.POS(:,1:2); % x and y coordinates of nodes
REG = msh.TRIANGLES(:,4); % physical region of each element
...
for i=1:nel
    ...
    REG_iel = REG(i);
    i_p = find(p(:,1)==REG_iel);
    i_t = find(t(:,1)==REG_iel);

    p_iel = p(i_p,2); % p for the given element
    t_iel = t(i_t,2); % t for the given element

    K_el = p_iel * (gradL'*gradL) * Area; % gradL is constant on the element!
    % rhs as shape function sampled at three nodes times t, assumed uniform
    % 1/3*t_i + 1/3*t_j + 1/3*t_k
    rhs_el = ones(3,1) * t_iel * Area/3;  
